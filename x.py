...
def download_pdf(url, save_path):
    try:
        ...
        if response.status_code == 200:
            ...
            logger.info(f"Successfully downloaded PDF from {url} to {save_path}")
            return True
        else:
...
def pdf_to_png(pdf_path, output_folder):
    try:
        ...
        logger.info(f"Successfully converted PDF {pdf_path} to PNG images in {output_folder}")
        return images
    except Exception as e:
        logger.error(f"Error converting PDF {pdf_path} to PNG images: {e}")
        return []
    ...
    try:
        with open(image_path, 'rb') as image_file:
            ...
            if response.status == 200:
                logger.info(f"Page {page_no} created for Catalog ID: {catalog_id}, Shop ID: {shop_id}")
            else:
                logger.error(f"Failed to create page {page_no} for Catalog ID: {catalog_id}, Shop ID: {shop_id}")
                logger.error(f"Response: {response.data.decode('utf-8')}")
    except Exception as e:
        logger.error(f"Error creating catalog page for Catalog ID: {catalog_id}, Shop ID: {shop_id}: {e}")
...
def process_row(row):
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(f"Failed to create catalog for {flyer_url}. Status code: {response.status_code}")
...
process_rows_parallel(df[10:])  # Process a subset of rows for testing
